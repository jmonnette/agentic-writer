...
- **Persona Library**: Collection of persona definitions in `.claude/personas/`
...
- Style alignment with `personal-voice` skill
...
│   └── skills/
│       └── personal-voice/  # Linguistic guardrails (generated by Stylist)
...
├── setup.sh                  # First-time initialization script
└── CLAUDE.md                 # This file
```
...
This ensures `FINAL.md` is always current while the full revision history is preserved in numbered versions.
...
- **Skill**: `personal-voice` for linguistic guardrails (generated by Stylist from your library)
- **Local Files**: `LIBRARY_INDEX.md`, `RESEARCH_INDEX.md`, `EXTERNAL_LIBRARY_INDEX.md`, `TOPICS.md`
...
6. **Attribution**: Any content drawn from `/external_library` must cite the original author, publication, and date
